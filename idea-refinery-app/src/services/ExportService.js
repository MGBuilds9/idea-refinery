...
Target Audience: ${ideaSpec.meta?.target_audience?.join(', ') || 'General users'}

## Features to Implement
${features.map(f => `- ${f.title}: ${f.user_story || 'No user story'}`).join('\n')}
...
${byComplexity.high.length ? byComplexity.high.map(renderFeature).join('') : '_No high-priority features defined_\n'}

---

## Phase 2: Secondary Features (Medium Priority)

${byComplexity.medium.length ? byComplexity.medium.map(renderFeature).join('') : '_No medium-priority features defined_\n'}

---

## Phase 3: Polish & Nice-to-Have (Low Priority)

${byComplexity.low.length ? byComplexity.low.map(f => `- ${f.title}${f.user_story ? `: ${f.user_story}` : ''}`).join('\n') : '_No low-priority features defined_'}

---

## Data Model

${dataModel.length ? dataModel.map(entity => {
...
${f.acceptance_criteria?.length ? `**Acceptance Criteria:**
...
${(ideaSpec.data_model || []).map(entity => `### ${entity.entity}
...
`;
  ...
  exportAll(ideaSpec, blueprint = '') {
    ...
    this.downloadFile(`${projectName}-implementation_plan.md`, this.toImplementationPlan(ideaSpec));
    this.downloadFile(`${projectName}-prompt.md`, this.toPromptMd(ideaSpec, blueprint));
  }
};
